            ...
            <tbody>
              {filteredProducts.map(product => (
                <tr key={product.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="px-5 py-4 text-sm font-mono text-indigo-600">{product.product_code}</td>
                  <td className="px-5 py-4 text-sm font-medium text-slate-800">{product.product_name}</td>
                  <td className="px-5 py-4 text-sm text-slate-500">{product.category || '-'}</td>
                  <td className="px-5 py-4 text-sm text-right text-slate-700">{formatNumber(product.purchase_cost)}원</td>
                  <td className="px-5 py-4 text-sm text-right text-slate-700">{formatNumber(product.packaging_cost)}원</td>
                  <td className="px-5 py-4 text-sm text-right font-semibold text-slate-800">{formatNumber(product.total_cost)}원</td>
                  <td className="px-5 py-4 text-sm text-center text-slate-500">{product.usage_count}</td>
                  <td className="px-5 py-4 text-center">
                    <button onClick={() => handleEdit(product)} className="p-1 hover:bg-slate-100 rounded text-sm mr-1">✏️</button>
                    <button onClick={() => handleDelete(product.id)} className="p-1 hover:bg-red-50 rounded text-sm">🗑️</button>
                  </td>
                </tr>
              ))}
              {filteredProducts.length === 0 && (
                <tr>
                  <td colSpan="8" className="px-5 py-12 text-center text-slate-400">
                    {search ? '검색 결과가 없습니다.' : '등록된 제품이 없습니다. 제품을 추가해주세요.'}
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">제품 코드</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">매입원가</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">포장비</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">부대비용</label>
                  <input
                    type="number"
