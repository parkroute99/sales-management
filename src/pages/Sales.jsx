...
function Sales() {
  ...
  useEffect(() => { fetchData() }, [filterChannel, filterDate])

  const fetchData = async () => {
    setLoading(true)
    let query = supabase
      .from('sales')
      .select('*, channels(channel_name, color_code), products(product_name, product_code), suppliers(supplier_name)')
      .order('sale_date', { ascending: false })
      .limit(500)

    if (filterChannel !== 'all') query = query.eq('channel_id', filterChannel)
    if (filterDate) {
      query = query.gte('sale_date', filterDate + '-01').lte('sale_date', filterDate + '-31')
    }
  ...
  const handleExcelDownload = () => {
    if (sales.length === 0) { alert('다운로드할 데이터가 없습니다.'); return }
    const excelData = sales.map(s => ({
      ...
      '수량': s.quantity,
      '판매가': Number(s.selling_price),
      '배송비수취': Number(s.shipping_fee_received),
      '총매출': Number(s.total_revenue),
      '상품원가': Number(s.product_cost),
      '수수료': Number(s.commission_amount),
      '배송비': Number(s.shipping_cost),
      '추가비용': Number(s.additional_fee),
      '총비용': Number(s.total_cost),
      '순이익': Number(s.net_profit),
      '마진률': Number(s.margin_rate) + '%',
      '메모': s.memo || '',
      '입력방법': s.input_method === 'EXCEL' ? '엑셀' : '수기',
    }))
    const ws = XLSX.utils.json_to_sheet(excelData)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, '매출내역')
    ws['!cols'] = [12,12,12,12,25,8,12,12,14,12,10,10,10,12,12,8,20,8].map(w => ({ wch: w }))
    XLSX.writeFile(wb, `매출내역_${filterDate || '전체'}_${new Date().toISOString().split('T')[0]}.xlsx`)
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <input type="month" value={filterDate} onChange={e => setFilterDate(e.target.value)}
          className="px-4 py-2.5 rounded-xl border border-slate-300 focus:border-indigo-500 outline-none text-sm" />
        {filterDate && <button onClick={() => setFilterDate('')} className="px-3 py-2.5 text-sm text-slate-500 hover:text-slate-700">초기화</button>}
            ...
            <tbody>
              {sales.map(sale => (
                <tr key={sale.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm font-medium text-slate-800 max-w-[200px] truncate">{sale.products?.product_name || '-'}</td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                      ...
                      Number(sale.margin_rate) >= 20 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'
                    }`}>{sale.margin_rate}%</span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      sale.input_method === 'EXCEL' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600'
                    }`}>{sale.input_method === 'EXCEL' ? '엑셀' : '수기'}</span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <button onClick={() => handleDelete(sale.id)} className="p-1 hover:bg-red-50 rounded text-sm">🗑️</button>
                  </td>
                </tr>
              ))}
              {sales.length === 0 && (
                <tr><td colSpan="11" className="px-4 py-12 text-center text-slate-400">매출 내역이 없습니다.</td></tr>
              )}
            </tbody>
          </table>
        </div>
