...
function Purchases() {
  ...
  const fetchData = async () => {
    ...
    if (filterDate) query = query.gte('purchase_date', filterDate + '-01').lte('purchase_date', filterDate + '-31')
    const { data: pData } = await query
    const { data: sData } = await supabase.from('suppliers').select('*').eq('is_active', true).order('sort_order')
    setPurchases(pData || []); setSuppliers(sData || []); setLoading(false)
  }

  const handleDelete = async (id) => {
    if (window.confirm('이 매입 기록을 삭제하시겠습니까?')) { await supabase.from('purchases').delete().eq('id', id); fetchData() }
  }

  const handleExcelDownload = () => {
    if (purchases.length === 0) { alert('데이터가 없습니다.'); return }
    const excelData = purchases.map(p => ({
      ...
      '수량': p.quantity, '매입단가': Number(p.purchase_price), '배송비': Number(p.shipping_cost),
      '부대비용': Number(p.additional_cost), '총매입금액': Number(p.total_amount),
      '메모': p.memo || '', '입력방법': p.input_method === 'EXCEL' ? '엑셀' : '수기',
    }))
    const ws = XLSX.utils.json_to_sheet(excelData)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, '매입내역')
    XLSX.writeFile(wb, `매입내역_${filterDate || '전체'}_${new Date().toISOString().split('T')[0]}.xlsx`)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <button onClick={handleExcelDownload} className="px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-medium hover:bg-emerald-700">📥 엑셀 다운로드</button>
      </div>
            ...
            <tbody>
              {purchases.map(p => (
                <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3"><span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium text-white"
                    style={{ backgroundColor: p.suppliers?.color_code || '#6366f1' }}>{p.suppliers?.supplier_name || '-'}</span></td>
                  <td className="px-4 py-3 text-sm font-medium text-slate-800 max-w-[200px] truncate">{p.products?.product_name || '-'}</td>
                  <td className="px-4 py-3 text-sm text-center">{p.quantity}</td>
                  <td className="px-4 py-3 text-sm text-right">{formatNumber(p.purchase_price)}</td>
                  <td className="px-4 py-3 text-sm text-right text-slate-500">{formatNumber(p.shipping_cost)}</td>
                  <td className="px-4 py-3 text-sm text-right font-semibold">{formatNumber(p.total_amount)}</td>
                  <td className="px-4 py-3 text-center"><span className={`text-xs px-2 py-1 rounded-full ${p.input_method === 'EXCEL' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600'}`}>{p.input_method === 'EXCEL' ? '엑셀' : '수기'}</span></td>
                  <td className="px-4 py-3 text-center"><button onClick={() => handleDelete(p.id)} className="p-1 hover:bg-red-50 rounded text-sm">🗑️</button></td>
                </tr>
              ))}
              {purchases.length === 0 && <tr><td colSpan="9" className="px-4 py-12 text-center text-slate-400">매입 내역이 없습니다.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
